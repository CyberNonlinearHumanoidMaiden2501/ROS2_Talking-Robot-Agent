...
- **vad_node + asr_node** (`vr_asr`) — Silero VAD sliding-window segmentation (emits ~192 ms speech windows on `/asr/utterance_audio`) feeding Qwen3-ASR 0.6B batched transcription (auto en/zh, GPU bf16, context-prompted; publishes `/asr/utterance`). No threads or queues in either node.
...
**M2 — Conversation core.** ✅ done — mock-verified end-to-end; physical conversation test pending (TEST_REPORT Test 4).
Brain turn-taking state machine (LISTENING → PROCESSING → SPEAKING), fully event-driven (async future chains, single-threaded executor); fast_llm_node serving ChatFast to deepseek-v4-flash (key from `~/deepseek-api-key`, HTTP/1.1 client); persona.yaml → system prompt with follow-user language rule; conversation store with history limit; barge-in via `/asr/utterance_audio` during playback → Play cancel → as-spoken truncation recording → regeneration. Dev-verified: utils unit tests, conversation e2e (fixed instruction + story + mid-reply interruption) all PASS.
