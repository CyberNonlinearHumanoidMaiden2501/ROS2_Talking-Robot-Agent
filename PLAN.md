...
- **vad_node + asr_node** (`vr_asr`) — Silero VAD with utterance segmentation (always-on trigger; publishes `/asr/speech_state` and trimmed `/asr/utterance_audio` segments) feeding Qwen3-ASR 0.6B transcription (auto en/zh, GPU bf16, synchronous callback; publishes `/asr/utterance`). No threads or queues in either node.
